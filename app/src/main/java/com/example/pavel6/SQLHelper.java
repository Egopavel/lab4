...
public class SQLHelper extends SQLiteOpenHelper{
    ...
    @Override
    public void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion){//Удаляет старую таблицу table1
        database.execSQL("DROP TABLE IF EXISTs table1");
        onCreate(database);
    }

    public Cursor getFullTable(){//возвращает курсор со всеми данными из таблицы table1
        SQLiteDatabase database = this.getReadableDatabase();
        return database.query("table1", new String[] {"_id", "route", "departureDate", "arrivalDate"},
                null, null, null, null, null);
    }

    public void addTrainRoute(String route, String departureDate, String arrivalDate) {//добавляет новую запись о маршруте поезда в таблицу.
    ...
    public void updateRoute(int id, String newRoute, String newDepartureDate, String newArrivalDate){//обновляет существующую запись по идентификатору (id).
        SQLiteDatabase db = getWritableDatabase();
    ...
    public void deleteRoute(int id) {//удаляет запись из таблицы по идентификатору (id).
        SQLiteDatabase db = getWritableDatabase();
